/*-----------------------------------

 SNIPPETS FOR ECOBRICKS.ORG/BRICKABLE.PHP


-----------------------------------*/


const es_Page_Translations = {
    
    "001-splash-title": "Diseño Brickable",
    "002-splash-subtitle": "Guía tu embalaje hacia su próxima vida mientras trabajas para eliminar completamente el plástico.",
    "003-splash-image-alt": "<img src=\"../icons/ecobrickable-icon-from-ecobricks-org-white.svg?v=1\" style=\"width: 70%\" alt=\"Icono designador brickable\">",
    
    "004-lead-paragraph": "El Icono Designador Brickable se puede colocar en plásticos de un solo uso para designar su próxima vida como un ecoladrillo. Nuestro Kit gratuito contiene todo lo que necesitas para guiar tu embalaje y productos hacia un ciclo de vida en espiral y verde",
    "005-page-paragraph-1": "La transición del plástico no es fácil. Por más que lo intentemos, en nuestra época, las regulaciones y mercados exigen el uso de plástico para casi todo. Sin embargo, el plástico en sí mismo no es el problema. Más bien, es nuestro pobre diseño y planificación. En particular: <i>el fallo en designar una próxima vida para nuestras creaciones de plástico que evite los peligros de promover aún más el consumo de plástico y aún más el procesamiento industrial del plástico.</i>",
    "006-page-paragraph-2": "Diseñar un producto como en espiral es el primer paso esencial en la transición del plástico de una empresa. Eliminarlo por completo es el segundo. La Designación Brickable permite a las empresas comenzar su <a href=\"transition.php\">transición del plástico hoy</a>.",
    "007-earthen-design-title": "Filosofía de Diseño Terrenal",
    "008-earthen-design-subtitle": "El contexto y el llamado para el diseño en espiral.",
    "009-reg-content-paragraph-1": "Para muchas empresas, hacer la transición a productos regenerativos es desalentador. Parece requerir grandes inversiones en fabricación, materiales, sistemas y certificaciones. Además, puede parecer que los materiales orgánicos alternativos aún no cumplen con los estándares de salud y seguridad y aún no son rentables.",
    "010-reg-content-paragraph-2": "Como paso de transición, los productos y empaques de plástico pueden hacerse brickables!",
    "011-reg-content-paragraph-3": "Diseñar un producto como brickable— y designarlo claramente como tal— es un primer paso hacia el diseño en espiral terrenal. Al designar un producto o empaque como brickable podemos mantener el plástico fuera del reciclaje industrial y la incineración que causan más problemas de los que resuelven. Fomentar el uso del plástico para convertirlo en un ecoladrillo muestra el camino hacia la próxima vida y propósito del plástico como un bloque de construcción reutilizable.",
    "012-reg-content-paragraph-4": "Los ecoladrillos y sus aplicaciones son en sí mismos completamente en espiral y basados en <a href=\"principles.php\">principios regenerativos</a>, permitiendo la reutilización indefinida y localizada del plástico hacia una <a href=\"sequest.php\">secuestro</a> a largo plazo. Tanto el Icono Designador Brickable como el ecoladrillo se basan en seguir el ejemplo de la Tierra en el cuidado del carbono y la ética de la contribución ecológica.",
    "013-reg-content-link-1": "🟢  <a href=\"spiral.php\">Aprende más sobre el Diseño en Espiral</a>",
    "014-reg-content-link-2": "🟢  <a href=\"sequest.php\">Aprende más sobre la Secuestración de Plástico</a>",
    "015-reg-content-link-3": "🟢  <a href=\"https://book.earthen.io/en/spirals.html\" target=\"_blank\">Aprende más sobre la Ética Terrenal de los Ciclos en Espiral</a>",
    "016-reg-content-link-4": "🟢  <a href=\"https://book.earthen.io/en/matter.html\" target=\"_blank\">Aprende más sobre la Ética Terrenal de la Concentración</a>",

    "017-brickable-icon-usage-criteria-title": "Criterios de Uso del Icono Brickable",
    "018-brickable-icon-usage-criteria-subtitle": "Los criterios materiales para los productos que usan el icono designador.",
    "019-gea-criteria-introduction": "La GEA ha establecido criterios simples que se deben cumplir al diseñar su producto o embalaje. Es obligatorio que su producto cumpla con estas estipulaciones para usar el Símbolo Designador Ecobrickable su producto o embalaje debe...",
    "020-criteria-corrosion": "...no contener sustancias que corroan el plástico PET con el tiempo.",
    "021-criteria-biodegradable": "...no contener ningún material biodegradable primario o secundario.",
    "022-criteria-size": "...ser fácilmente desmontable, roto o cortado en partes que puedan caber dentro del cuello de 22 mm de diámetro de una botella PET estándar.",
    "023-criteria-disassembly": "...maximizar la facilidad de desmontaje para este propósito.",
    "024-criteria-sharpness": "...no tener protuberancias afiladas que puedan perforar un ecobrick desde dentro mientras se está compactando.",
    "025-criteria-chemicals": "...no contener productos químicos reactivos, o si los tiene, estos componentes deben estar claramente marcados como no brickables.",
    "026-side-module-1-alt": "<img src=\"../webp/brickable-kit.webp?v=1\" width=\"300\" style=\"width:90%;\" alt=\"Icono del Kit de Diseño Brickable\" loading=\"lazy\">",
    "027-side-module-1-title": "Comenzar",
    "028-side-module-1-subtitle": "Descarga el Kit Designador Brickable hoy.",
    "029-side-module-1-button": "🚀 Descargar",
    "030-side-module-button-caption": "Archivo ZIP | 3,2 MB",

    "032-opener-header-text": "See How Its Used",
    "033-opener-subheader-text": "Companies and corporations around the world use the designator icon for their non-recyclable packaging.",
    "034-usage-description": "The designator icon can be used directly in packaging designs or added as sticker to pre-existing packaging and products.",
    "035-usage-examples-intro": "Here's some examples of our usage:",
    "036-example-link-1": '<a href="photos/brickable-1-716px.jpeg"><img src="../photos/brickable-1-716px.jpeg" style="width:30%;" alt="A black sticker used on a chocolate powder plastic bag" loading="lazy"></a>',
    "037-example-link-2": '<a href="photos/brickable-2-716px.jpeg"><img src="../photos/brickable-2-716px.jpeg" style="width:30%;" alt="Integrated into the packaging of a Marimas juice product" loading="lazy"></a>',
    "038-example-link-3": '<a href="photos/brickable-3-716px.jpeg"><img src="../photos/brickable-3-716px.jpeg" style="width:30%;" alt="A stick on a home based business health drink product" loading="lazy"></a>',
    "049-opener-header-text": "Creative Commons License",
    "050-opener-subheader-text": "Conditions for graphic use",
    "051-license-description": "The Ecobrickable Designator & Stickers are licensed under a Creative Commons Attribution-NoDerivatives 4.0 International License. This is to ensure the standardized and resonant usage of the designators by committed and resonant companies and individuals. See copyright notice and terms below.",
    "052-license-link": '<a rel="license" href="http://creativecommons.org/licenses/by-nd/4.0/"><img alt="Creative Commons License" style="border-width:0" src="https://i.creativecommons.org/l/by-nd/4.0/88x31.png"></a>',
    "053-license-text": '<a rel="license" href="http://creativecommons.org/licenses/by-nd/4.0/">Creative Commons Attribution-NoDerivatives 4.0 International License</a>.',
    "039-get-started-header": "Get Started!",
    "040-kit-description": "Download the Brickable Designator Kit today to use on your legacy plastic products and packaging. The ZIP package includes:",
    "041-kit-contents-icon-sticker": "Icon & Sticker Design Elements (Vector / PDF)",
    "042-kit-contents-recommended-colors": "Recommended colors (Vector / PDF)",
    "043-kit-contents-example-sticker": "Example of A3 sticker printing (JPEG)",
    "044-kit-contents-resolution-icons": "High, medium and low resolution icons (PNG)",
    "045-kit-contents-terms": "Terms and Conditions of use (TXT)",
    "046-kit-contents-license": "Creative Commons Attribution-NoDerivatives 4.0 International License (TXT/PNG)",
    "047-download-btn": '<a class="action-btn" href="https://nextcloud.ecobricks.org/index.php/s/rL3yFwETbcLF9dR" target="_blank">🚀 Download Kit</a>',
    "048-file-info": "ZIP file | 3.2MB"

    
};
