 ...
 function presentSearchResults(posts, query) {
    ...
    if (outputPosts.length == 0) {
       ...
       for (var k = 0; k < outputPosts.length; k++) {
          resultsContainer.innerHTML += "<div class=\"tc-item\"><div id='result_" + k + "' style=\"display:flex; text-align:left; padding: 20px;\"><div class=\"chapter_pic\" style=\"width=100px; margin-right:10px;display:block;\"><img src=\"" + outputPosts[k].night_image_url + "\" width=\"100px\" height=\"100px\"></div><div class=\"chapter-name-search\"><b style=\"font-size:x-large;font-family:'Arvo';margin-bottom:12px;display:block;\"><a href='" + outputPosts[k].url + "'>" + outputPosts[k].title + "</b><span style=\"font-size:smaller;color:var(--drop-cap)!important;margin-top:10px;display:block;\">" + outputPosts[k].section + "  |  " + outputPosts[k].language + "  |  " + outputPosts[k].id + "  |  " + outputPosts[k].lang_key + "</span><span style=\"font-size:medium;font-family:'CooperLt',serif;margin-top:10px;display:block;\">" + outputPosts[k].description + "</span><span style=\"font-size:smaller;color:grey;margin-top:10px;display:block;\">ecobricks.org/" + outputPosts[k].language + "/" + outputPosts[k].url + "</span></a></div>";
       }
    }
 }
...
function handleKeyPress(event) {
   if (event.keyCode === 13) { // 13 is the key code for the enter key
      ...
      siteSearch(['indexes/glossary.json', 'indexes/page-index.json']); // Call your search function
   }
}
