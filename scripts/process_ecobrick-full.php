...
<div id="ecobrick-being-processed">
    ...
    if (isset($data['records']) && count($data['records']) > 0) {
        ...
        // Now, update the record to set field_2492 and field_2525 to 'Yes'
        $update_url = "https://api.knack.com/v1/objects/object_2/records/$knack_record_id";
        $update_data = json_encode(['field_2492' => 'Yes', 'field_2525' => 'Yes']);
    ...
    // PART 3: Data Retrieval and Database Insertion
    if (isset($data['records']) && count($data['records']) > 0) {
        $success = true;
        $errors = [];
        ...
        foreach ($data['records'] as $record) {
            if (isset($record['field_73']) && $record['field_73'] == $ecobrick_unique_id) {
                ...
                $maker_id = $record['field_335_raw'][0]['id'] ?? ''; // Add the new field
    ...
    echo "<div class='message'>Now closing database session, and moving to next ecobrick...</div>";
