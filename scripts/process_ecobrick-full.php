...
if (isset($data['records']) && count($data['records']) > 0) {
    ...
    foreach ($data['records'] as $record) {
        if (isset($record['field_73']) && $record['field_73'] == $ecobrick_unique_id) { // Updated to use $ecobrick_unique_id
            ...
            $knack_record_id = $record['field_335_raw'][0]['id'] ?? ''; // Add the new field
    ...
    if (!$record_found) {
        echo "<script>if(confirm('No records found for the given Ecobrick ID. Do you want to proceed to the next ecobrick?')) { window.location.href = 'process_ecobrick_full.php'; }</script>";
    } elseif (!$success) {
        echo "<script>if(confirm('Error: " . implode(", ", $errors) . ". Do you want to proceed to the next ecobrick?')) { window.location.href = 'process_ecobrick_full.php'; }</script>";
    }
}
