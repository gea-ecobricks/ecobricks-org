...
if (isset($data['records']) && count($data['records']) > 0) {
    ...
    foreach ($data['records'] as $record) {
        if (isset($record['field_1361']) && $record['field_1361'] == $training_id) {
            ...
            $training_photo5_main = isset($record['field_2178_raw']) ? $record['field_2178_raw']['url'] : '';
            ...
            if ($check_stmt->num_rows > 0) {
                ...
                // Prepare and bind
                $stmt = $conn->prepare("INSERT INTO tb_trainings (training_id, training_title, training_logged, no_participants, lead_trainer, training_photo0_main, training_photo1_main, training_photo2_main, training_photo3_main, training_photo4_main, training_photo5_main, training_type, briks_made, est_plastic_packed, location_full, training_summary, training_agenda, training_success, training_challenges, training_lessons_learned) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                if ($stmt === false) {
                    die("<script>alert('Prepare failed: " . htmlspecialchars($conn->error) . "');</script>");
                }
                $stmt->bind_param("ssssssssssssssssssss", $training_id, $training_title, $training_logged, $no_participants, $lead_trainer, $training_photo0_main, $training_photo1_main, $training_photo2_main, $training_photo3_main, $training_photo4_main, $training_photo5_main, $training_type, $briks_made, $est_plastic_packed, $location_full, $training_summary, $training_agenda, $training_success, $training_challenges, $training_lessons_learned);
            ...
            $record_details = "
                ...
                <p><img src='$training_photo4_main' alt='Feature Photo 2' style='max-width: 400px;' title='$training_photo4_main' /></p>
                <p><img src='$training_photo5_main' alt='Feature Photo 2' style='max-width: 400px;' title='$training_photo5_main' /></p>
            ";
            break;
        }
    }
