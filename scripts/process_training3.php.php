...
<html>
<head>
    <title>Training Import Progress</title>
    <style>
        body {
...
<body>
    <h1>Training Import Progress</h1>
    <div id="progress">
        <?php
        // PART 1 of the code
        ...
        if ($conn->connect_error) {
            echo "<div class='alert'>Connection failed: " . $conn->connect_error . "</div>";
            ob_flush(); flush();
            exit;
        }
        ...
        if ($response === false) {
            $error = curl_error($ch);
            echo "<div class='alert'>Error fetching data from Knack API: " . htmlspecialchars($error) . "</div>";
            curl_close($ch);
            ob_flush(); flush();
            exit;
        }
        ...
        echo "<div class='message'>Knack API Request URL: " . htmlspecialchars($url) . "</div>";
        echo "<div class='message'>Knack API Response: " . htmlspecialchars($response) . "</div>";
        ob_flush(); flush();
        ...
        if (isset($data['records']) && count($data['records']) > 0) {
            ...
            if (!$record_found) {
                echo "<div class='alert'>No records found for the given Training ID.</div>";
                ob_flush(); flush();
            } elseif (!$success) {
            ...
            echo "<div class='alert'>No records found in the Knack database.</div>";
            ob_flush(); flush();
        }
